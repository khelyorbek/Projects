...
def logout():
    ...
    # Step Two: Fix Logout
    # On logout, it should flash a success messag
    flash("Logout complete. We will miss you!", 'success')
    do_logout()
...
def profile():
    ...
    # It should show a form with the following: username, email, image_url, header_image_url, bio, password
    form = UserEditForm(obj=g.user)

    if form.validate_on_submit():
        # It should check that that password is the valid password for the user
        # Not passing form.username.data because if the user typed in a new username, it would try to authenticate using the new user id and WILL ALWAYS FAIL
...
def like_post(post_id):
    ...
    liked_post = Message.query.get_or_404(post_id)
...
def homepage():
    ...
    if g.user:
        # Getting a list of ids from following list
        # Adding the id of the user at the end so our own posts also show
